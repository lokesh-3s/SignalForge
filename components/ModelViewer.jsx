...
const ModelViewer = ({
  ...
  const containerStyle = width && height 
    ? { width, height, touchAction: 'pan-y pinch-zoom' }
    : { width: '100%', height: '100%', touchAction: 'pan-y pinch-zoom' };
  ...
  return (
    ...
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute top-2 right-2 sm:top-4 sm:right-4 z-10 cursor-pointer px-2 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm border border-white rounded-xl bg-transparent text-white hover:bg-white hover:text-black transition-colors"
        >
          Take Screenshot
        </button>
      )}
